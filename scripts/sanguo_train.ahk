...
LoadConfig() {
    ...
    ; 加载训练士兵任务配置
    totalLoops := Integer(IniRead("..\config\settings.ini", "Tasks", "TrainLoops", "10"))
    coords := ParseCoords(IniRead("..\config\settings.ini", "Tasks", "TrainCoords", "900,360|900,360|1145,510|1180,720|1250,800|1180,720|1130,800|1180,720|910,800"))
...
Main() {
    ...
    ToolTip("三国训练士兵脚本已启动，按F1开始/暂停，按F2停止")
...
CheckStatus() {
    ...
    if (isRunning) {
        if (currentLoop < totalLoops) {
            ...
            Sleep(30600000)  ; 每轮之间等待8.5小时 (8.5 * 60 * 60 * 1000 = 30600000毫秒)
        } else {
            isRunning := false
